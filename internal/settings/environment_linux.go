...
func GetEnvironment() (environment HandlerEnvironment, _ error) {
	...
	if err != nil {
		return environment, errors.WithStack(fmt.Errorf("vmextension: cannot find base directory of the running process: %v", err))
	}
	paths := []string{
	...
	for _, p := range paths {
		...
		if err != nil && !os.IsNotExist(err) {
			return environment, errors.WithStack(fmt.Errorf("vmextension: error examining HandlerEnvironment at '%s': %v", p, err))
		} else if err == nil {
			b = o
			break
		}
	...
	if b == nil {
		return environment, errors.WithStack(fmt.Errorf("vmextension: Cannot find HandlerEnvironment at paths: %s", strings.Join(paths, ", ")))
	}
...
func parseEnvironmentManifest(b []byte) (he HandlerEnvironment, _ error) {
	...
	if err := json.Unmarshal(b, &hf); err != nil {
		return he, errors.WithStack(fmt.Errorf("vmextension: failed to parse handler env: %v", err))
	}
	if len(hf) != 1 {
		return he, errors.WithStack(fmt.Errorf("vmextension: expected 1 config in parsed HandlerEnvironment, found: %v", len(hf)))
	}
	return hf[0], nil
}
