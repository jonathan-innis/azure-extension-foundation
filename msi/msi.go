...
func (p *provider) GetMsi() (Msi, error) {
	...
	if code != 200 {
		return msi, errors.WithStack(fmt.Errorf("unable to get msi, metadata service response code %v", code))
	}

	err = json.Unmarshal(body, &msi)
	if err != nil {
		return msi, errors.WithStack(fmt.Errorf("unable to deserialize metadata service response"))
	}
